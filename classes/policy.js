...
export default class Policy {
  ...
  restrict(req, res) {
    ...
    if (this.logging) {
      chalk.info(
        `Routing   ${req.id} to ${this.controller.name}#${this.action}`
      );
    }
    ...
    if (this.policy) {
      ...
      if (this.policies[this.policy]) {
        ...
        this.policies[this.policy](
          req,
          res,
          this.acceptor(req, res),
          this.rejector(req, res)
        );
      } else {
        this.debug("Policy: %s not found [error] %s", this.policy, req.id);
        chalk.error(
          `Policy Error: ${req.id || ""} The policy "${
            this.policy
          }" does not exist, therefore the request was denied`
        );
        this.policies.onFailure && this.policies.onFailure(req, res);
      }
    } else {
  ...
  runControllerMethod(req, res) {
    ...
    let controller = new this.controller(
      req,
      res,
      this.server.redis,
      this.server.websockets
    );
    if (controller[this.action]) {
      controller[this.action]();
    } else {
      chalk.error(
        `Routing Error: ${req.id || ""} Can not route request to "${
          req.controller
        }#${req.action}" because ${req.action} does not exist on ${
          req.controller
        }`
      );
