...
import object, { explode as _explode } from '../namespace/object.js';
let {
  hasPath, set, get,
  extend, clone,
...
describe("Object Methods", function () {
  ...
  it("extend:: should extend the src object with many objects", function () {
    ...
    let ext = {two: 2};
    let ext2 = {three: 3};
    deepEqual(extend(src, ext, ext2), {one: 1, two: 2, three: 3});
  });
  ...
  it("extend:: should overwrite src when keys are same", function () {
    ...
    let ext = {one: 2};
    let ext2 = {three: 3};
    deepEqual(extend(src, ext, ext2), {one: 2, three: 3});
  });
  ...
  it("extend:: should overwrite keys in order of args when keys are same", function () {
    ...
    let ext2 = {two: 3};
    let ext3 = {two: 4};
    deepEqual(extend(src, ext, ext2, ext3), {one: 1, two: 4});
  });

  it("hasPath:: should return a boolean value if an object contains a path", function () {
    let src = {one: 1};
    equal(hasPath(src, 'one'), true);
    equal(hasPath(src, 'one.value.at.nowhere'), false);
  });

  it("clone:: should return a new object", function () {
    let src = {one: 1, two: {a: 1}};
    deepEqual(clone(src), {one: 1, two: {a: 1}});
  });
  ...
  it("createShallowInvertedClone:: should create a shallow inverted clone", function () {
    ...
    deepEqual(x, {A: 'a', B: 'b'});
    equal(x.a, undefined);           // Non-inverted keys should not be present
    deepEqual(o, {a: 'A', b: 'B'});     // Original should be non-inverted
  });
  ...
  it("select:: should select keys from an object", function () {
    ...
    deepEqual(fields, {name: 'fred', email: 'me@mail.com'});
  });
  ...
  it("drop:: should drop keys from an object", function () {
    let o = { name: 'fred', email: 'me@mail.com', password: 'secretSquirrel' };
    drop(o, 'password');
    deepEqual(o, {name: 'fred', email: 'me@mail.com'});
  });
  ...
  it("selectCombination:: should select values from multiple objects and create a new one", function () {
    ...
    deepEqual(o, { name: 'fred', email: 'me@mail.com', last_visit: b.last_visit, likes : 'stuff', knows: 'things'});
  });
  ...
  it("format:: should format values properly (1)", function () {
    ...
    deepEqual(o, { name: 'fred', email: 'me@mail.com', stuff: 'a', value : 'value'});
  });
  ...
  it("format:: should format values properly (3)", function () {
    ...
    deepEqual(o, { name: 'fred', email: 'me@mail.com', data: { stuff: { a: 'a'}}, value : 'value'});
  });
